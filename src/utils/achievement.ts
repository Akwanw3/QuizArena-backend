...
import { IAchievementDefinition } from '../types/Index';
  ...
  try {
    ...
    for (const achievement of ACHIEVEMENTS) {
      ...
      if (achievement.condition(stats)) {
        // Unlock achievement
        const unlocked = await Achievement.create({
          userId,
 ...
 */
export const getAchievementProgress = async (userId: string) => {
  try {
    const user = await User.findById(userId);
    if (!user) return null;
    ...
    return {
      ...
      progress: Math.round((unlockedIds.length / ACHIEVEMENTS.length) * 100),
