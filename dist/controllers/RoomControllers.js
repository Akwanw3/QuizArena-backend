...
exports.deleteRoom = exports.updateRoomSettings = exports.kickPlayer = exports.toggleReady = exports.leaveRoom = exports.joinRoom = exports.getRoomByCode = exports.getPublicRooms = exports.createRoom = void 0;
...
exports.createRoom = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!category || !difficulty || !numberOfQuestions || !timePerQuestion) {
        throw new ErrorHandler_1.AppError('Please provide all room settings', 400);
    }
    // Validate ranges
    if (numberOfQuestions < 5 || numberOfQuestions > 20) {
        throw new ErrorHandler_1.AppError('Number of questions must be between 5 and 20', 400);
    }
    if (timePerQuestion < 5 || timePerQuestion > 60) {
        throw new ErrorHandler_1.AppError('Time per question must be between 5 and 60 seconds', 400);
    }
    // Step 3: Generate unique room code
    const roomCode = await Room_1.default.generateRoomCode();
 ...
 */
exports.getPublicRooms = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const rooms = await Room_1.default.find(filter)
 ...
 * Public route
 */
exports.getRoomByCode = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
...
exports.joinRoom = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (room.isFull()) {
        throw new ErrorHandler_1.AppError('Room is full (maximum 10 players)', 400);
    }
    // Step 3: Check if game already started
    if (room.status !== 'waiting') {
        throw new ErrorHandler_1.AppError('Cannot join - game already started', 400);
    }
    // Step 4: Check if user already in room
    const existingPlayer = room.getPlayer(req.user?.userId);
    if (existingPlayer) {
        throw new ErrorHandler_1.AppError('You are already in this room', 400);
    }
    ...
    server_1.io.to(roomCode).emit('room:updated', room);
    res.status(200).json({
        success: true,
...
exports.leaveRoom = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (playerIndex === -1) {
        throw new ErrorHandler_1.AppError('You are not in this room', 400);
    }
    ...
    if (room.players.length === 0 || room.hostId === req.user?.userId) {
        await Room_1.default.findByIdAndDelete(room._id);
        // Emit room deleted event
        server_1.io.to(roomCode).emit('room:deleted', {
 ...
 * Protected route
 */
exports.toggleReady = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const { roomCode } = req.params;
    // Find room
    const room = await Room_1.default.findOne({ roomCode: roomCode.toUpperCase() });
    if (!room) {
 ...
 * Protected route
 */
exports.kickPlayer = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!room.isHost(req.user?.userId)) {
        throw new ErrorHandler_1.AppError('Only host can kick players', 403);
    }
    // Check if trying to kick themselves
    if (userId === req.user?.userId) {
        throw new ErrorHandler_1.AppError('Cannot kick yourself', 400);
    }
    // Find player to kick
    const playerIndex = room.players.findIndex((p) => p.userId === userId);
    if (playerIndex === -1) {
        throw new ErrorHandler_1.AppError('Player not found in room', 404);
    }
    // Remove player
    ...
    // Emit events
    server_1.io.to(roomCode).emit('room:updated', room);
    server_1.io.to(userId).emit('room:kicked', {
 ...
 * Protected route
 */
exports.updateRoomSettings = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    const { roomCode } = req.params;
    const { category, difficulty, numberOfQuestions, timePerQuestion, isPublic } = req.body;
    // Find room
    const room = await Room_1.default.findOne({ roomCode: roomCode.toUpperCase() });
    ...
    // Check if requester is host
    if (!room.isHost(req.user?.userId)) {
        throw new ErrorHandler_1.AppError('Only host can update settings', 403);
    }
    // Check if game already started
    if (room.status !== 'waiting') {
        throw new ErrorHandler_1.AppError('Cannot update settings - game already started', 400);
    }
    // Update settings
    if (category)
...
exports.deleteRoom = (0, ErrorHandler_1.asyncHandler)(async (req, res) => {
    ...
    // Check if requester is host
    if (!room.isHost(req.user?.userId)) {
        throw new ErrorHandler_1.AppError('Only host can delete room', 403);
    }
    // Delete room
    await Room_1.default.findByIdAndDelete(room._id);
    // Emit room deleted event
